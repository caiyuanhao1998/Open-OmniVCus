...
# 处理完每一个 mask 之后，将 mask_list 里面的每个 frame 都存下来
def process_segments_video_save_mask(video_segments, frames_np_lst_selected, save_folder_path):
    mask_list = []
    for frame_segments in range(len(video_segments)):
        cur_frame_segment = video_segments[frame_segments]
        frame_mask = np.zeros_like(frames_np_lst_selected[frame_segments])[:,:,0]
        for i in range(len(cur_frame_segment)):
            mask = cur_frame_segment[i+1]
            frame_mask = frame_mask * (1-mask) + (frame_mask * 0 + 1 + i) * mask
        cv2.imwrite(save_folder_path + f'mask_{frame_segments}.png', frame_mask*20)
        mask_list.append(frame_mask*20)
...
# all_part_indexes = [i for i in range(440, 1060)]      #seg-1
# all_part_indexes = [i for i in range(1000, 1400)]      #seg-2
# all_part_indexes = [i for i in range(503, 1200)]      #seg-1
# all_part_indexes = [i for i in range(1200, 1300)]      #seg-2
# all_part_indexes = [i for i in range(1300, 1398)]      #seg-3
# all_part_indexes = [i for i in range(1398, 1496)]      #seg-4
# all_part_indexes = [i for i in range(1496, 1594)]      #seg-5
all_part_indexes = [i for i in range(1594, 1690)]      #seg-6

s3_cus_label_dir = 'yuanhao_cai/VideoEdit/label/video2seg/'
...
print("num of part_indexes: ", len(part_indexes))
...
for part_i in part_indexes:
    ...
    save_root = '/mnt/localssd/yuanhaoc/video2seg/'
    ...
    for i in tqdm(range(len(video_names))):
        try:
            ...
            # stx()
            save_folder_path = save_dir + name.replace('.mp4', '_mask/')
            os.makedirs(save_folder_path, exist_ok=True)
            mask_list = process_segments_video_save_mask(video_segments, frames_np_lst, save_folder_path)

            caption_frame_mask = mask_list[idx_caption_frame]
            cv2.imwrite(save_dir + name.replace('.mp4', '_caption_frame_mask.png'), caption_frame_mask)

            # 从 save_folder_path 里面随机读取一张 mask
            # mask_reload_path = random.choice(os.listdir(save_folder_path))
            # stx()
            # mask_reload = cv2.imread(save_folder_path + mask_reload_path)
            # print("np.unique(mask_reload): ", np.unique(mask_reload))
            # stx()

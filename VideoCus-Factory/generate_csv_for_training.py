...
for dpo_id in range(16):
    ...
    root_path = f"T2V_data/label/vidgen_filter_gemini_part_0_dpo_{dpo_id}"
    mp4_root = "T2V_data/VIDGEN-1M_unzip"
    out_csv = os.path.join(out_dir, f"vidgen_filter_gemini_part_0_dpo_{dpo_id}.csv")
...
merged_csv = os.path.join(out_dir, "vidgen_filter_gemini_part_0_all.csv")

all_rows = []
header = None

for dpo_id in range(16):
    csv_path = os.path.join(out_dir, f"vidgen_filter_gemini_part_0_dpo_{dpo_id}.csv")

...
s3_dir = 'yuanhao_cai/VideoEdit/label/video2seg/'
...
for i in tqdm(range(len(folder_names))):
    ...
    # stx()
    # stx()
    caption_names = [f for f in file_names if f.endswith('.json')]
...
local_json_path = save_dir + 'video2seg.json'
with open(local_json_path, 'w') as file:
    json.dump(total_caption_names[test_num:], file, indent=4)


# 将所有的 video_names 存到一个 json 文件里
local_json_path_test = save_dir + 'video2seg_test.json'

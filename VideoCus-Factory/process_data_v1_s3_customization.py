...
# part_indexes = [11,]
# all_part_indexes = [i for i in range(0,65)]
# all_part_indexes = [i for i in range(65,250)]
# all_part_indexes = [i for i in range(250,450)]
# all_part_indexes = [i for i in range(450,600)]  #3
# all_part_indexes = [i for i in range(600,750)]  #4
# all_part_indexes = [i for i in range(750,975)]  #5

# cus_idx = 2

# all_part_indexes = [i for i in range((cus_idx-1)*100, cus_idx*100)]  # 6

# all_part_indexes = [i for i in range(1007,1690)]    # cust-1

# all_part_indexes = [i for i in range(1209,1479)]    # cust-1

all_part_indexes = [i for i in range(1218,1690)]    # cust

s3_cus_label_dir = 'yuanhao_cai/VideoEdit/label/video_customize/'
...
print("part_indexes:", part_indexes)
print("number of part_indexes:", len(part_indexes))
...
def check_mask_size(mask):
    ...
    # if mask_portion > 0.005 and mask_portion < 0.7 and is_foreground_touching_edges(mask_bin):
    # 要不要 filter 掉呢？
    if mask_portion > 0.01 and mask_portion < 0.36 and is_foreground_touching_edges(mask_bin):
        return 1
    else:
        return 0
...
for part_i in part_indexes:
    ...
    save_root = '/mnt/localssd/yuanhaoc/video_customize/'
    ...
    for i in tqdm(range(len(video_names))):
        try:
            ...
            # ==== select valid mask =====
            # 找出 mask_list 里面所有共有的 label
            mask_sets = [set(np.unique(mask)) for mask in mask_list]
            common_index = set.intersection(*mask_sets)
            common_index.discard(0)
            # stx()
            
            # filter 的规则: check 每一帧的 mask 是否满足条件
            # 当全部都满足条件的时候再将这个 mask 存下来
            has_saved_mask = False
            for i in common_index:
                save_flag = True
                for j in range(len(mask_list)):
                    cur_mask = mask_list[j]
                    mask = np.zeros_like(cur_mask)
                    mask[cur_mask == i] = 1
                    # stx()
                    if check_mask_size(mask) == 0:
                        save_flag = False
                        break
                # stx()
                # 只将 caption frame 的 mask 存下来
                # 将对应的 mask_idx 存入 json_dict 中，后续训练的时候可以将 entities crop 出来
                if save_flag:
                    if not has_saved_mask:
                        name_caption_frame_mask = name.replace('.mp4', '_mask.png')
                        caption_frame_mask = mask_list[idx_caption_frame]
                        # caption_frame_mask = np.stack([caption_frame_mask,caption_frame_mask,caption_frame_mask],-1)
                        # print(np.unique(caption_frame_mask))
                        # stx()
                        cv2.imwrite(save_dir + name_caption_frame_mask, caption_frame_mask)
                        # print("save path:", save_dir + name_caption_frame_mask)
                        # reload_mask = cv2.imread(save_dir + name_caption_frame_mask)
                        # print(np.unique(reload_mask))
                        # stx()
                        has_saved_mask = True
                    json_dict['common_entities'].append(int(i))
                    # 使用 caption_frame_mask 分割 caption frame 来验证这个 entity 是否能与 mask idx 对应上
                    cur_bin_mask = (caption_frame_mask == i).astype(int)
                    seg_entity = caption_frame * cur_bin_mask[:,:,np.newaxis]
                    seg_entity_path = save_dir + name.replace('.mp4', f'_entity_{i}.png')
                    cv2.imwrite(seg_entity_path, seg_entity[...,::-1])
            # stx()


            # ==============================
            #  保存 caption frame
            caption_frame_save_path = save_dir + name.replace('.mp4', '_caption_frame.png')
            # caption frame rgb 转 bgr
            cv2.imwrite(caption_frame_save_path, cv2.cvtColor(caption_frame, cv2.COLOR_RGB2BGR))

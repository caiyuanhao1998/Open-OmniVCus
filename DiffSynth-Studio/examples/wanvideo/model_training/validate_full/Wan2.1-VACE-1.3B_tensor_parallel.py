...
class InferTask:
    ...
    out_path: str = "video_Wan2.1-VACE-1.3B.mp4"
...
def read_prompt_txt(path: str) -> str:
    ...
    with open(path, "r", encoding="utf-8") as f:
        s = f.read()
    # remove all occurrences of "IMG1 " then strip
...
# Lightning module
# -----------------------------
class LitWanVACE13BInfer(pl.LightningModule):
    ...
    def _init_pipe_if_needed(self):
        ...
        if is_rank0():
            print(f"[rank {get_rank()}] init pipe on {device_str}")
    ...
    def configure_model(self):
        ...
        if is_rank0():
            print("[rank0] Tensor parallel applied to pipe.vace in configure_model().")
...
def main():
    ...
    parser.add_argument("--out", type=str, default="video_Wan2.1-VACE-1.3B.mp4")
    ...
    # 兼容：也允许直接传 prompt
    parser.add_argument("--prompt", type=str, default="a girl turning around in front of an old airplane")

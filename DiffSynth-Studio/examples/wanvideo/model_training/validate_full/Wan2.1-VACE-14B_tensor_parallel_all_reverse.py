...
def build_tasks_from_test_dir(
    ...
    for sub_name, sub_path in subdirs:
        # ✅ 2) 如果输出下已有对应结果则跳过（兼容 mp4 或结果文件夹 两种产物）
        out_mp4 = os.path.join(out_dir, f"{sub_name}.mp4")
        out_folder = os.path.join(out_dir, sub_name)

        mp4_exists = os.path.isfile(out_mp4) and os.path.getsize(out_mp4) > 0
        folder_exists = os.path.isdir(out_folder)

        if mp4_exists or folder_exists:
            skipped += 1
            if is_rank0():
                reason = []
                if mp4_exists:
                    reason.append(f"mp4 exists: {out_mp4}")
                if folder_exists:
                    reason.append(f"folder exists: {out_folder}")
                print(f"[rank0] skip {sub_name} ({'; '.join(reason)})")
            continue
    ...
    if is_rank0():
        print(
            f"[rank0] build_tasks: total_subdirs={len(subdirs)}, tasks_to_run={len(tasks)}, skipped={skipped}"
        )

...
# -----------------------------
# Small task wrapper (same style as 1.3B)
# -----------------------------
@dataclass
class InferTask:
    prompt: str
    ...
    out_path: str = "video_Wan2.2-VACE-Fun-A14B.mp4"
...
def _modules_to_parallelize(pipe: Any) -> List[nn.Module]:
    """
    Wan2.2 A14B 常见：vace/vace2（以及可能的 dit/dit2）
    都做 TP。
    """
    mods: List[nn.Module] = []
...
# -----------------------------
# Lightning module (Wan2.2 A14B with VRAM management)
# -----------------------------
class LitWanVACEA14BInfer(pl.LightningModule):
    ...
    def _init_pipe_if_needed(self):
        ...
        # ✅ VRAM 管理：把 preparing/computation 绑到本 rank 的 device
        vram_config = {
        ...
        # ✅ vram_limit：按本 rank 的卡算（留 2GB buffer）
        ...
        # ✅ 加载 finetune：high->vace, low->vace2（都从 CPU load，省显存）
    ...
    def predict_step(self, batch: InferTask, batch_idx: int, dataloader_idx: int = 0):
        ...
        # ✅ 始终按 num_frames 截断（你说 num_frames 需要输入）
        ...
        with torch.inference_mode(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            out_video = self.pipe(
                ...
                num_frames=task.num_frames,   # ✅ 始终传入
                seed=task.seed,
                tiled=task.tiled,
            )
...
def main():
    ...
    # ✅ 你要求 num_frames 需要输入：设为 required=True

...
def _tp_plan_for_module(mod: nn.Module) -> Dict[str, Any]:
    ...
    - attention block: 尽量启用 SequenceParallel（对 token 维做 Shard）
    这里用“通用名字匹配”，适配大多数 DiT/Transformer 实现。
    """
    plan: Dict[str, Any] = {}
...
def _find_backbone_module(pipe: Any) -> nn.Module:
    """
    尝试在 pipe 里找到“主干网络”，不同版本可能叫：
    - dit / transformer / unet / model / diffusion_model / backbone 等
    找到就返回，否则报错。
    """
    candidates = ["dit", "transformer", "unet", "model", "diffusion_model", "backbone", "net"]
    for key in candidates:
        if hasattr(pipe, key) and isinstance(getattr(pipe, key), nn.Module):
            return getattr(pipe, key)

    for name, val in pipe.__dict__.items():
    ...
    raise RuntimeError(
        "找不到可 TP 的 backbone 模块。请打印 pipe.__dict__.keys() 看看主干叫啥，"
        "然后把 candidates 里加上对应名字，或直接用 getattr(pipe, 'XXX')。"
    )


def apply_tensor_parallel_to_pipe(pipe: Any) -> None:
    """
    对 pipeline 的主干做 tensor parallel。
    注意：TP 只需要在每个 rank 初始化完模型后调用一次。
    """
    backbone = _find_backbone_module(pipe)
...
class LitWanVACE13B(pl.LightningModule):
    def __init__(self, args):
        ...
        self.args = args
        self.pipe = None
        self._tp_applied = False  # 防止重复 parallelize
    ...
    def _init_pipe_if_needed(self):
        ...
        self.pipe = WanVideoPipeline.from_pretrained(
            ...
            # ✅ 这里强烈建议开 use_usp=True（Wan 自带的序列并行，比通用 SequenceParallel 更稳）
            # use_usp=True,
        )
    ...
    def setup(self, stage: str):
        # setup 里只做“建模 + 放对 device”，不要做 parallelize（Lightning 要求在 configure_model 里做）
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./tp_outputs")
    parser.add_argument("--seed", type=int, default=1)
    args = parser.parse_args()

    # ⚠️ barrier 需要 dist 初始化后才可用；这里由 lightning strategy 初始化
    # 所以我们在下载后做一次“尽力而为”的 barrier：没 init 就跳过
    if is_rank0():
